...
export async function POST(request: Request) {
  try {
    ...
    const printers = normalizePrinterInventory((body?.printers ?? []).map((printer) => ({
      ...
      driver: typeof printer.driver === 'string' ? printer.driver : undefined,
    ...
    const device = await prisma.tenantDeviceRegistry.upsert({
      where: { tenantId_deviceId: { tenantId: tenant.tenantId, deviceId } },
      update: {
        branchId: body?.branchId ?? tenant.branchId ?? null,
