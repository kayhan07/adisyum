...
export async function GET(request: Request) {
  ...
  const stream = new ReadableStream({
    start(controller) {
      async function emit() {
        try {
          controller.enqueue(encoder.encode(`event: live-operations\ndata: ${JSON.stringify({ ...(await getLiveOperationsSnapshot()), generatedAt: new Date().toISOString() })}\n\n`));
        } catch (error) {
          controller.enqueue(encoder.encode(`event: error\ndata: ${JSON.stringify({ message: error instanceof Error ? error.message : 'stream failed' })}\n\n`));
        }
      }
      void emit();
      timer = setInterval(() => { void emit(); }, 5000);
    },
    cancel() {
