...
| Server-side printer proxy still needs local agent compatibility | WORKING | VPS/server proxy may need to probe the existing local bridge fallback without exposing those URLs to browser chunks. | `app/api/printers/local-agent/route.ts`, `app/api/printers/local-agent/print/route.ts` | Existing server-controlled `3001` fallback remains server-side only. No audit bypass was added. | Source grep shows remaining `3001` only under server API routes and Windows tray shortcut; built client chunks are clean. |
