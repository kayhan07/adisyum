...
### CORE POS BACKBONE FINAL HARDENING FINDINGS

| Başlık | Durum | Kök neden | Değişen dosyalar | Yapılan fix | Manuel QA notu | Kalan risk |
| --- | --- | --- | --- | --- | --- | --- |
| Masalar | WORKING | Masalar ekranındaki toplam yardımcısı satış fiyatını net tutar gibi yorumlayıp tekrar KDV ekliyordu. Bu durum masa toplamını ve doluluk görünümünü API ile tutarsızlaştırabiliyordu. | `components/floor-workspace.tsx` | Masa toplamı artık girilen KDV dahil satış fiyatlarının toplamıdır. Doluluk durumu authoritative açık adisyon toplamına bağlı kalır. | 100 TL ürün eklenmiş masanın 100 TL ve dolu göründüğünü, ödeme sonrası boş kaldığını canlıda doğrula. | Canlı tarayıcı QA gerekir. |
| Adisyon | WORKING | DB sipariş toplamı ekleme ve miktar güncellemede satır toplamına ikinci kez KDV ekliyordu. | `app/api/pos/table-orders/route.ts`, `components/order-composer.tsx`, `scripts/trace-pos-insertion.ts` | `total` brüt satış toplamına eşitlendi. `taxTotal` raporlama için brüt toplamın içinden ayrıştırılır; fiyat üstüne eklenmez. | Ürün fiyatı 100 TL gir, ekle, miktarı değiştir, masadan çıkıp geri gir ve refresh yap. Birim fiyat 100 TL kalmalı. | Canlı DB smoke testi gerekir. |
| Ürünler | WORKING | Mevcut recovery guardları demo ürün fallback, tenant-scoped katalog ve silinen ürünün geri dönmesini zaten denetliyordu. | `scripts/verify-pos-backbone.mjs` | Backbone guardına tenant-linked katalog hydrate ve demo masa koruması eklendi. | Yeni tenant'ta ürün listesinin boş olduğunu, eklenen ürünün refresh sonrası kaldığını ve silinen ürünün POS katalogdan kalktığını doğrula. | Manuel hızlı ürün ekleme QA gerekir. |
| Ödeme | PARTIAL | Tam ödeme authoritative API üzerinden idempotent kapanıyor. Ancak parçalı tahsilat dalları hâlâ local runtime journal davranışı kullanıyor. | `scripts/verify-pos-backbone.mjs` | Guard tam ödemenin önce server kapanışı, sonra local journal yazımı sırasını ve duplicate payment kontrolünü doğrular. | Tam ödeme al, refresh yap ve masanın boş kaldığını doğrula. | Parçalı tahsilatı server-side authoritative mutasyona taşımak ayrı kontrollü iş olarak açık kalıyor. |
| Yazıcı / Agent etkisi | WORKING | Agent yokluğu daha önce kaydetmeyi engelleyebiliyordu; kayıtlı yazıcılar offline durumda görünmez olabiliyordu. | `scripts/verify-pos-backbone.mjs` | Backbone guard save-before-print sırasını ve offline kayıtlı yazıcı fallback kodlarını doğrular. | Agent kapalıyken kaydet/yazdır yap; adisyon korunmalı ve görünür uyarı çıkmalı. | Windows cihaz QA gerekir. |
| Demo kalıntıları ve tenant izolasyonu | WORKING | Production hydrate yolunda demo ve yabancı tenant snapshot riski sürekli regresyon kontrolü gerektiriyor. | `scripts/verify-pos-backbone.mjs`, `package.json` | `npm run pos:backbone` eklendi; demo masa seed kilidi, tenant mismatch snapshot reddi, tenant-linked hydrate ve yazıcı fallback sözleşmesi tek guardda doğrulanır. | DB dry-run çıktısını production ortamında ayrıca incele. | Production DB residue audit sonucu operatör tarafından değerlendirilmelidir. |
